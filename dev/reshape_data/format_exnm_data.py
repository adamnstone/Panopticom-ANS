...
# colors for arcs above the globe based on subject area
subject_areas_to_colors = {
...
# hard-coded lab codes from the Expert Network Map
...
# dictionary to store coordinates for each lab code
...
# dictionary to map the unrecognized codes to recognized codes
unrecognized_code_to_recognized_code = {}
originally_unrecognized_codes = []

# all of the fab lab codes
all_codes = names_from_codes.keys()

# make a list of the unrecognized codes
for c in all_codes:
...
# try to map all of the unrecognized codes to equivalent recognized codes
...
# get coordinates from any fab lab code (recognized or unrecognized)
...
# determines the equivalency of two different codes/slugs for the same fab lab
...
# i counts how many of the references have been successfully mapped to two geographical areas for the target and source lab
i = 0
for l in exnm_data['links']:
    # get lab from the documentation url
    ...
    # convert the lab codes to recognized codes
    ...
    # if it is a reference within the same lab, do not add it to the dataset (won't be an arc)
    if rec_source_lab == rec_target_lab: continue

    # if the source lab has not already been added to the dataset, add a new dictionary with target labs;
    # otherwise, add one to the reference's subject area for the target lab dictionary of the source lab
    subj = l['topic']
...
for source_lab in output_data.keys():
    for target_lab in output_data[source_lab]:
        ...
        for subj in output_data[source_lab][target_lab]:
            ...
            current_dict['color'].append(subject_areas_to_colors[subj])
